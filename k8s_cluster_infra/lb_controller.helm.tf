...
provider "helm" {
  kubernetes {
    host                   = aws_eks_cluster.careerhub.endpoint
    cluster_ca_certificate = base64decode(aws_eks_cluster.careerhub.certificate_authority.0.data)
...
module "lb_controller_role" {
  ...
  provider_url = replace(aws_eks_cluster.careerhub.identity.0.oidc.0.issuer, "https://", "")
...
resource "helm_release" "release" {
  ...
  dynamic "set" {
    for_each = {
      ...
      "vpcId"                 = local.network_output.vpc_id
      "image.repository"      = "602401143452.dkr.ecr.${local.network_output.region}.amazonaws.com/amazon/aws-load-balancer-controller"
