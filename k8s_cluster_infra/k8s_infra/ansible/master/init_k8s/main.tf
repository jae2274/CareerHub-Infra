module "init_kubeadm" {
  ...
  playing_name = "init_kubeadm_${var.group_name}"
...
module "install_network_plugin" {
  ...
  log_dir_path = var.log_dir_path
  group_name   = var.group_name
  playing_name = "install_network_plugin_${var.group_name}"
...
module "install_helm" {
  ...
  log_dir_path = var.log_dir_path
  group_name   = var.group_name
  playing_name = "install_helm_${var.group_name}"
...
module "install_metrics_server" {
  ...
  log_dir_path = var.log_dir_path
  group_name   = var.group_name
  playing_name = "install_metrics_server_${var.group_name}"
